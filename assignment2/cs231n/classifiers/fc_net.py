...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1,self.num_layers):
            self.params['W'+str(i)] = weight_scale*np.random.randn(curr_dim, hidden_dims[i-1]) 
            #np.random.normal(0, weight_scale, (curr_dim, hidden_dims[i-1])) 
            self.params['b'+str(i)] = np.zeros(hidden_dims[i-1])
            curr_dim = hidden_dims[i-1]
        self.params['W'+str(self.num_layers)] =  weight_scale*np.random.randn(curr_dim, num_classes) 
        #np.random.normal(0, weight_scale, (curr_dim, num_classes)) 
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
            ...
            if self.normalization != None:
                ...
                out, c = affine_norm_relu_forward(out, W, b, gamma, beta, self.bn_params[i-1],self.normalization)
            else:
                out, c = affine_relu_forward(out, W, b) 
            if self.use_dropout:
                out, c_dropout = dropout_forward(out, self.dropout_param)
                c = c,c_dropout
            cache[i] = c
            
        W, b = self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)]        
        scores, c = affine_forward(out, W, b)
        cache[self.num_layers] = c
        ...
        for i in reversed(range(1,self.num_layers)):            
            if self.use_dropout:
                c,c_dropout = cache[i]
                dx = dropout_backward(dx, c_dropout) 
            else:
                c = cache[i]
            if self.normalization != None:
                dx, dW, db, dgamma, dbeta  = affine_norm_relu_backward(dx, c, self.normalization)
                grads['gamma'+str(i)] = dgamma
                grads['beta'+str(i)] = dbeta
            else:
                dx, dW, db = affine_relu_backward(dx, c)            
...
def affine_norm_relu_forward(x, w, b, gamma, beta, norm_param, normalization):
    """
    ...
    if normalization=='batchnorm':
        a_norm, norm_cache = batchnorm_forward(a, gamma, beta, norm_param)
    elif normalization=='layernorm':
        a_norm, norm_cache = layernorm_forward(a, gamma, beta, norm_param)
